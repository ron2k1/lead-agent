...
- **Status:** **v1.1.0 - runtime gate ACTIVE + production-grade libraries** (shipped 2026-05-07). All **12** pinned runtime-gate components built and integrity-chained via `lib/install-hook.ps1`; trust anchor stamped via the bootstrap installer (`install.ps1`); end-to-end -Verify probe is green on the dogfood machine. **Library status**: `lib/secret-scan.ps1` and `lib/jsonl-watcher.ps1` are now **production-grade** (canonical 15-pattern regex set; HMAC-SHA256-signed scan-pass manifest; role-prefix neutralization + secret scrub on JSONL tail) -- the v1.0 stubs are gone. **Wiring status**: BUILDER pre-push hook **wiring** that calls `secret-scan.ps1` and OVERWATCH ingest-loop **wiring** that consumes `jsonl-watcher.ps1` are still stubbed and deferred to v1.2 (originally promised for v1.1.1; v1.1.1 became a distribution-first patch and the wiring was re-deferred to v1.2 alongside the v1.2 task-board work); the libraries themselves are ready to be called. Mode readiness: ADVISOR + TOOLSMITH = READY for daily use; BUILDER + OVERWATCH = PARTIAL (library production-grade, wiring pending). The deny-by-default invariant holds because the hook itself is fully wired - the wiring gaps only block autonomous sub-features, not the gate. New in v1.1.0: F-01 launch.ps1 -Force orphan-lock auto-recovery via PID + Win32_Process.CreationDate stale-detection; F-02 runner.ps1 3-layer lock release (try/finally + Register-EngineEvent + P/Invoke SetConsoleCtrlHandler); launch.cmd -Force forwarding for double-click recovery; W3-NEW3 deny-on-detect for shell control tokens (`;`, `&&`, `||`, `|`, `&`, `>`, `<`, command-substitution sigils); pin-set expanded 9 -> 12 (added secret-scan.ps1, jsonl-watcher.ps1, runner.ps1). v1.1.0 closes 8 Wave 3b findings (3 BLOCKERs + 4 MAJORs + 1 MINOR) plus Wave 3c convergence (1 BLOCKER + 3 MAJORs) plus Wave 3d (1 BLOCKER + 3 MAJORs). Walkback: 17 commits across Waves 3b/3c/3d. v1.0 archive banner: shipped 2026-05-06 with 9 pinned components, v1.0 closes V8-8 schema/contract-grounding BLOCKER from v0.9 ceiling-rule cycle plus 8 v0.9 residuals; full v0.9-final history in section 15.
...
4. **Inside the new wt tab, runner.ps1 runs:**
   ...
   **(B5 v1.1.1 carve-out):** the v1.0.x minimum PATH (`SYSTEMROOT\System32;SYSTEMROOT`) broke ~14 MCPs that need `node`/`npx`/`python`/`uv` on PATH (caught 2026-05-04 in lieutenant-spawn debug). v1.1.1 expands the reconstructed minimum to ALSO include the parent directories of those tools as resolved by `Get-Command` BEFORE the scrub runs (see `runner.ps1:126-148`). The SE-E2 invariant still holds because: (a) tool-dir resolution happens at scrub time using the authenticated PATH the lead launcher inherits, NOT a PATH the lieutenant could mutate; (b) the lead still resolves `claude.exe` / `git.exe` / `gh.exe` via `$manifest.absPaths.*`, never through PATH; (c) only the resolved tool dirs are added back, the rest of the parent PATH is dropped. Net effect: same shadowing-resistance for the binaries that matter, plus enough PATH for MCP child processes to find their interpreters.
